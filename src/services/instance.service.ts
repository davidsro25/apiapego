...
export class InstanceService {
  ...
  static async get(idOrName: string): Promise<Instance | null> {
    const row = await queryOne<Instance>(
      'SELECT * FROM instances WHERE id = $1 OR name = $1',
      [idOrName]
    )
    if (!row) return null
  ...
  static async create(dto: CreateInstanceDto): Promise<Instance> {
    ...
    const id = uuidv4()
    const apiKey = `whaapi_${uuidv4().replace(/-/g, '')}`
    ...
    // Inicia a conexão Baileys em background
  ...
  static async updateSettings(idOrName: string, settings: Record<string, any>): Promise<Instance> {
    ...
    const [updated] = await query<Instance>(
      `UPDATE instances SET settings = $2, updated_at = NOW() WHERE id = $1 RETURNING *`,
